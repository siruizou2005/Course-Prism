...
export function useFollowedReviews(pagination: Pagination | null) {
  const { data, error } = useSWR<PaginationApiResult<Review>>(
    pagination ? `/api/review/?page=${pagination.page}&size=${pagination.pageSize}&notification_level=${NotificationLevel.FOLLOW}` : null,
    fetcher
  );
  return {
    reviews: data,
